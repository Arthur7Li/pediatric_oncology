...
char = char.replace("await page.goto('/en/support');", "await page.goto('en/support');");
fs.writeFileSync('e2e/charities.spec.ts', char);

// Fix glossary.spec.ts
let glo = fs.readFileSync('e2e/glossary.spec.ts', 'utf8');
glo = glo.replace("await page.goto('/en/glossary');", "await page.goto('en/glossary');");
...
journey = journey.replace('await page.goto("en/journey/1-diagnosis");', 
...
journey = journey.replace('// Navigate Back: Step 2 -> Step 1',
